...
public class ConfigE {
	...
	public ConfigE(String courseNames[], Integer num) {
		JCheckBox check[] = new JCheckBox[num + 5];
		JButton OK = new JButton("OK");
		Integer y = 30;
		JLabel label = new JLabel("E DAY");
		label.setBounds(185, 5, 100, 30);
		label.setFont(new Font("Pristina", Font.BOLD, 25));
		label.setForeground(Color.WHITE);
		JLabel instruct = new JLabel("(Please tick the E Day's classes)");
		...
		OK.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				makeDir.mkdirs();
				try {
					FileOutputStream foutCourse = new FileOutputStream(new File(dir + File.separator + "E_Course.txt"));
				
					for(Integer j = 1; j <= 14; j++) {
						String dirtmp = "data" + File.separator + j.toString() + File.separator + "E";
