...
public class E extends JPanel {
	...
	public E() {
		try {
			...
			JOptionPane.showMessageDialog(null, "Error Occurred in E Day!", "Warning", JOptionPane.WARNING_MESSAGE);
			e.printStackTrace();
		}
		...
		try {
			String dir = "data" + File.separator + "Courses" + File.separator + "E_Course.txt";
			InputStream tmp = new FileInputStream(new File(dir));
			Scanner scr = new Scanner(tmp);
			String input;
			for(Integer j = 1; j <= num; j++) {
				String dirtmp = "data" + File.separator + UI.comboState + File.separator + "E" + File.separator + j.toString() + ".txt";
					...
					public void itemStateChanged(ItemEvent em) {
						...
						for(Integer i = 1; i <= num; i++) {
							if(em.getSource() == check[i]) {
								...
								String dir = "data" + File.separator + UI.comboState + File.separator + "E" + File.separator + i.toString() + ".txt";
