...
public class A extends JPanel {
	...
	public A() {
		try {
			...
			JOptionPane.showMessageDialog(null, "Error Occurred in A Day!", "Warning", JOptionPane.WARNING_MESSAGE);
			e.printStackTrace();
		}
		...
		try {
			String dir = "data" + File.separator + "Courses" + File.separator + "A_Course.txt";
			InputStream tmp = new FileInputStream(new File(dir));
			Scanner scr = new Scanner(tmp);
			String input;
			for(Integer j = 1; j <= num; j++) {
				String dirtmp = "data" + File.separator + UI.comboState + File.separator + "A" + File.separator + j.toString() + ".txt";
					...
					public void itemStateChanged(ItemEvent em) {
						...
						for(Integer i = 1; i <= num; i++) {
							if(em.getSource() == check[i]) {
								...
								String dir = "data" + File.separator + UI.comboState + File.separator + "A" + File.separator + i.toString() + ".txt";
